...
void benchNTT(PoseidonContext &context, BatchEncoder &encoder, double scale, size_t degree) {
...
ParametersLiteral auto_generate_bfv_params(uint32_t level) {
    uint32_t log_n;
    ...
    ParametersLiteralDefault bfv_param_literal(BFV, 1 << log_n, poseidon::sec_level_type::tc128);
    bfv_param_literal.set_log_modulus(log_q, log_p);

    return bfv_param_literal;
}
...
int main()
{
    ...
    for (int i = 2; i < 21; i += 2) {
        // 测试不同乘法深度下的参数生成
        ParametersLiteral bfv_param_literal = auto_generate_bfv_params(i);
        ...
        // benchEncoder(encoder, bfv_param_literal.plain_modulus().value());
        benchNTT(context, encoder, scale, bfv_param_literal.degree());
