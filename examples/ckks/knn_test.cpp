...
std::map<int, bool> is_finished;       // 任务是否完成
std::map<int, std::vector<int>> pre_task;   // 前置任务
...
int main(int argc, char *argv[])
{
    ...
    thread_pool.enqueue_prepare(std::move(op1_check), std::move(op1));

    vector<int> step = {100, 200, 300, 600, 1200, 2500, 5000};
    auto op2 = [&kgen, &step, &rot_keys] ()
    {
    ...
    thread_pool.enqueue_prepare(std::move(op2_check), std::move(op2));
