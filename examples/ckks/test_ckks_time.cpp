...
void benchEncoder(CKKSEncoder &encoder, double scale) {
    ...
    for (size_t i = 0; i < slot_count; i++) {
        values.push_back({ ((double)rand() / RAND_MAX) * 2 - 1, ((double)rand() / RAND_MAX) * 2 - 1 });
    }
...
void benchNTT(PoseidonContext &context, CKKSEncoder &encoder, double scale, size_t degree) {
    ...
    // 准备数据
    size_t slot_count = encoder.slot_count();
    vector<double> vector_data(slot_count, 1.0);
    
    Plaintext pt_plain;
    encoder.encode(vector_data, scale, pt_plain);
    ...
    for (int i = 0; i < N; i++) {
        Ciphertext res;
        evaluator->ntt_inv(ct1, res);
    }
    cout << "Ciphertext INTT Time: " << chrono::duration<double, micro>(chrono::high_resolution_clock::now() - start).count() / N << " us" << endl;

    // vector<uint64_t> poly(degree);

    // 创建 CoeffIter (包装原始指针)
    // util::CoeffIter ci(poly.data());
    // auto &ntt_table = context.crt_context()->small_ntt_tables()[0];
    // start = chrono::high_resolution_clock::now();
    // for (int i = 0; i < N; i++) {
    //     util::ntt_negacyclic_harvey(ci, ntt_table); 
    // }
    // auto end = chrono::high_resolution_clock::now();
    // double avg_time = chrono::duration_cast<chrono::nanoseconds>(end - start).count() / 10000.0;
    // printf("Poseidon Core NTT (N=%zu): %.2f ns\n", degree, avg_time);

    // start = chrono::high_resolution_clock::now();
    // for (int i = 0; i < N; i++) {
    //     util::inverse_ntt_negacyclic_harvey_lazy(ci, ntt_table); 
    // }
    // end = chrono::high_resolution_clock::now();
    // avg_time = chrono::duration_cast<chrono::nanoseconds>(end - start).count() / 10000.0;
    // printf("Poseidon Core INTT (N=%zu): %.2f ns\n", degree, avg_time);
}

void benchEvaluator(PoseidonContext &context, CKKSEncoder &encoder, double scale) {
    ...
    Plaintext pt_plain;
    encoder.encode(vector_data, scale, pt_plain); // 对应 Lattigo 的 plaintext
    ...
    cout << "MulRelin_Cipher: " << chrono::duration<double, micro>(chrono::high_resolution_clock::now() - start).count() / N << " us" << endl;

    // --- 6. Rescale ---
    start = chrono::high_resolution_clock::now();
    for (int i = 0; i < N; i++) {
        Ciphertext res;
        evaluator->rescale(ct1, res);
    }
    cout << "Rescale: " << chrono::duration<double, micro>(chrono::high_resolution_clock::now() - start).count() / N << " us" << endl;

    // --- 7. Rotate ---
    start = chrono::high_resolution_clock::now();
    for (int i = 0; i < N; i++) {
        Ciphertext res;
        evaluator->rotate(ct1, res, 1, gal_keys);
    }
    cout << "Rotate: " << chrono::duration<double, micro>(chrono::high_resolution_clock::now() - start).count() / N << " us" << endl;
}



ParametersLiteral auto_generate_ckks_params(uint32_t level) {
    uint32_t log_n;
    ...
    ParametersLiteral literal{CKKS, log_n, log_n - 1, 40, 5, 0, 0, {}, {}};
    literal.set_log_modulus(log_q, log_p);
...
int main()
{
    ...
    for (int i = 2; i < 21; i += 2) {
        // 测试不同乘法深度下的参数生成
        ParametersLiteral ckks_param_literal = auto_generate_ckks_params(i);
        ...
        // benchEncoder(encoder, scale);
        benchNTT(context, encoder, scale, ckks_param_literal.degree());
        benchEvaluator(context, encoder, scale);
    }
