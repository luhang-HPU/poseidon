...
namespace ckks_dag24 {

constexpr int kParallelBranchCount = 24;
...
void print_internal_omp_regions(
    const std::vector<omp_trace::RegionSnapshot> &regions) {
  for (const auto &region : regions) {
    std::cout << "    [libomp] " << region.name << " | omp_threads "
              << format_int_set(region.omp_threads, "(none)")
              << " | omp_places "
              << format_int_set(region.omp_places, "(unavailable)")
              << " | cpu_cores "
              << format_int_set(region.cpu_cores, "(unavailable)") << " | hits "
              << region.hits << " | team " << region.max_team_size
...
void print_dag_trace(const std::vector<DagTraceItem> &trace) {
  ...
  std::cout << "  parallel_24_branches TOTAL_TIME: " << branch_total << " ms"
            << std::endl;
  std::cout << "  parallel_24_branches CORES: " << format_cores(branch_cores)
            << std::endl;
...
build_reference(const std::vector<std::complex<double>> &msg_a,
  ...
  branches.push_back(parallel_branch_ref(msg_a, msg_b, 1, 4));
  branches.push_back(parallel_branch_ref(msg_a, msg_c, 2, 8));
  branches.push_back(parallel_branch_ref(msg_a, msg_d, 4, 16));
  branches.push_back(parallel_branch_ref(msg_b, msg_c, 8, 32));
  branches.push_back(parallel_branch_ref(msg_b, msg_d, 16, 1));
  branches.push_back(parallel_branch_ref(msg_c, msg_d, 32, 2));
  branches.push_back(parallel_branch_ref(msg_a, msg_b, 2, 4));
  branches.push_back(parallel_branch_ref(msg_a, msg_c, 4, 8));
  branches.push_back(parallel_branch_ref(msg_a, msg_d, 8, 16));
  branches.push_back(parallel_branch_ref(msg_b, msg_c, 16, 32));
  branches.push_back(parallel_branch_ref(msg_b, msg_d, 32, 1));
  branches.push_back(parallel_branch_ref(msg_c, msg_d, 1, 2));
  branches.push_back(parallel_branch_ref(msg_a, msg_b, 4, 8));
  branches.push_back(parallel_branch_ref(msg_a, msg_c, 8, 16));
  branches.push_back(parallel_branch_ref(msg_a, msg_d, 16, 32));
  branches.push_back(parallel_branch_ref(msg_b, msg_c, 32, 1));
  branches.push_back(parallel_branch_ref(msg_b, msg_d, 1, 2));
  branches.push_back(parallel_branch_ref(msg_c, msg_d, 2, 4));
  branches.push_back(parallel_branch_ref(msg_a, msg_b, 8, 16));
  branches.push_back(parallel_branch_ref(msg_a, msg_c, 16, 32));
  branches.push_back(parallel_branch_ref(msg_a, msg_d, 32, 1));
  branches.push_back(parallel_branch_ref(msg_b, msg_c, 1, 2));
  branches.push_back(parallel_branch_ref(msg_b, msg_d, 2, 4));
  branches.push_back(parallel_branch_ref(msg_c, msg_d, 4, 8));
...
void ckks_single_thread_workload(EvaluatorCkksBase &ckks_eva,
                                 ...
                                 Ciphertext &result,
                                 std::vector<DagTraceItem> *trace) {
  Ciphertext branch_00;
  Ciphertext branch_01;
  Ciphertext branch_02;
  Ciphertext branch_03;
  Ciphertext branch_04;
  Ciphertext branch_05;
  Ciphertext branch_06;
  Ciphertext branch_07;
  Ciphertext branch_08;
  Ciphertext branch_09;
  Ciphertext branch_10;
  Ciphertext branch_11;
  Ciphertext branch_12;
  Ciphertext branch_13;
  Ciphertext branch_14;
  Ciphertext branch_15;
  Ciphertext branch_16;
  Ciphertext branch_17;
  Ciphertext branch_18;
  Ciphertext branch_19;
  Ciphertext branch_20;
  Ciphertext branch_21;
  Ciphertext branch_22;
  Ciphertext branch_23;

  branch_00 = parallel_branch(ckks_eva, relin_keys, galois_keys, scale, ct_a,
                              ct_b, 1, 4, "branch_00", trace);
  branch_01 = parallel_branch(ckks_eva, relin_keys, galois_keys, scale, ct_a,
                              ct_c, 2, 8, "branch_01", trace);
  branch_02 = parallel_branch(ckks_eva, relin_keys, galois_keys, scale, ct_a,
                              ct_d, 4, 16, "branch_02", trace);
  branch_03 = parallel_branch(ckks_eva, relin_keys, galois_keys, scale, ct_b,
                              ct_c, 8, 32, "branch_03", trace);
  branch_04 = parallel_branch(ckks_eva, relin_keys, galois_keys, scale, ct_b,
                              ct_d, 16, 1, "branch_04", trace);
  branch_05 = parallel_branch(ckks_eva, relin_keys, galois_keys, scale, ct_c,
                              ct_d, 32, 2, "branch_05", trace);
  branch_06 = parallel_branch(ckks_eva, relin_keys, galois_keys, scale, ct_a,
                              ct_b, 2, 4, "branch_06", trace);
  branch_07 = parallel_branch(ckks_eva, relin_keys, galois_keys, scale, ct_a,
                              ct_c, 4, 8, "branch_07", trace);
  branch_08 = parallel_branch(ckks_eva, relin_keys, galois_keys, scale, ct_a,
                              ct_d, 8, 16, "branch_08", trace);
  branch_09 = parallel_branch(ckks_eva, relin_keys, galois_keys, scale, ct_b,
                              ct_c, 16, 32, "branch_09", trace);
  branch_10 = parallel_branch(ckks_eva, relin_keys, galois_keys, scale, ct_b,
                              ct_d, 32, 1, "branch_10", trace);
  branch_11 = parallel_branch(ckks_eva, relin_keys, galois_keys, scale, ct_c,
                              ct_d, 1, 2, "branch_11", trace);
  branch_12 = parallel_branch(ckks_eva, relin_keys, galois_keys, scale, ct_a,
                              ct_b, 4, 8, "branch_12", trace);
  branch_13 = parallel_branch(ckks_eva, relin_keys, galois_keys, scale, ct_a,
                              ct_c, 8, 16, "branch_13", trace);
  branch_14 = parallel_branch(ckks_eva, relin_keys, galois_keys, scale, ct_a,
                              ct_d, 16, 32, "branch_14", trace);
  branch_15 = parallel_branch(ckks_eva, relin_keys, galois_keys, scale, ct_b,
                              ct_c, 32, 1, "branch_15", trace);
  branch_16 = parallel_branch(ckks_eva, relin_keys, galois_keys, scale, ct_b,
                              ct_d, 1, 2, "branch_16", trace);
  branch_17 = parallel_branch(ckks_eva, relin_keys, galois_keys, scale, ct_c,
                              ct_d, 2, 4, "branch_17", trace);
  branch_18 = parallel_branch(ckks_eva, relin_keys, galois_keys, scale, ct_a,
                              ct_b, 8, 16, "branch_18", trace);
  branch_19 = parallel_branch(ckks_eva, relin_keys, galois_keys, scale, ct_a,
                              ct_c, 16, 32, "branch_19", trace);
  branch_20 = parallel_branch(ckks_eva, relin_keys, galois_keys, scale, ct_a,
                              ct_d, 32, 1, "branch_20", trace);
  branch_21 = parallel_branch(ckks_eva, relin_keys, galois_keys, scale, ct_b,
                              ct_c, 1, 2, "branch_21", trace);
  branch_22 = parallel_branch(ckks_eva, relin_keys, galois_keys, scale, ct_b,
                              ct_d, 2, 4, "branch_22", trace);
  branch_23 = parallel_branch(ckks_eva, relin_keys, galois_keys, scale, ct_c,
                              ct_d, 4, 8, "branch_23", trace);

  const std::vector<const Ciphertext *> branches{
      &branch_00, &branch_01, &branch_02, &branch_03, &branch_04, &branch_05,
      &branch_06, &branch_07, &branch_08, &branch_09, &branch_10, &branch_11,
      &branch_12, &branch_13, &branch_14, &branch_15, &branch_16, &branch_17,
      &branch_18, &branch_19, &branch_20, &branch_21, &branch_22, &branch_23};
  merge_parallel_branches(ckks_eva, relin_keys, galois_keys, scale, branches,
                          result, trace);
}

void ckks_manual_parallel_24_workload(
  ...
  parallel.go([&]() {
    branches[0] =
        parallel_branch(ckks_eva, relin_keys, galois_keys, scale, ct_a, ct_b, 1,
                        4, "branch_00", &branch_traces[0]);
  });
  parallel.go([&]() {
    branches[1] =
        parallel_branch(ckks_eva, relin_keys, galois_keys, scale, ct_a, ct_c, 2,
                        8, "branch_01", &branch_traces[1]);
  });
  parallel.go([&]() {
    branches[2] =
        parallel_branch(ckks_eva, relin_keys, galois_keys, scale, ct_a, ct_d, 4,
                        16, "branch_02", &branch_traces[2]);
  });
  parallel.go([&]() {
    branches[3] =
        parallel_branch(ckks_eva, relin_keys, galois_keys, scale, ct_b, ct_c, 8,
                        32, "branch_03", &branch_traces[3]);
  });
  parallel.go([&]() {
    branches[4] =
        parallel_branch(ckks_eva, relin_keys, galois_keys, scale, ct_b, ct_d,
                        16, 1, "branch_04", &branch_traces[4]);
  });
  parallel.go([&]() {
    branches[5] =
        parallel_branch(ckks_eva, relin_keys, galois_keys, scale, ct_c, ct_d,
                        32, 2, "branch_05", &branch_traces[5]);
  });
  parallel.go([&]() {
    branches[6] =
        parallel_branch(ckks_eva, relin_keys, galois_keys, scale, ct_a, ct_b, 2,
                        4, "branch_06", &branch_traces[6]);
  });
  parallel.go([&]() {
    branches[7] =
        parallel_branch(ckks_eva, relin_keys, galois_keys, scale, ct_a, ct_c, 4,
                        8, "branch_07", &branch_traces[7]);
  });
  parallel.go([&]() {
    branches[8] =
        parallel_branch(ckks_eva, relin_keys, galois_keys, scale, ct_a, ct_d, 8,
                        16, "branch_08", &branch_traces[8]);
  });
  parallel.go([&]() {
    branches[9] =
        parallel_branch(ckks_eva, relin_keys, galois_keys, scale, ct_b, ct_c,
                        16, 32, "branch_09", &branch_traces[9]);
  });
  parallel.go([&]() {
    branches[10] =
        parallel_branch(ckks_eva, relin_keys, galois_keys, scale, ct_b, ct_d,
                        32, 1, "branch_10", &branch_traces[10]);
  });
  parallel.go([&]() {
    branches[11] =
        parallel_branch(ckks_eva, relin_keys, galois_keys, scale, ct_c, ct_d, 1,
                        2, "branch_11", &branch_traces[11]);
  });
  parallel.go([&]() {
    branches[12] =
        parallel_branch(ckks_eva, relin_keys, galois_keys, scale, ct_a, ct_b, 4,
                        8, "branch_12", &branch_traces[12]);
  });
  parallel.go([&]() {
    branches[13] =
        parallel_branch(ckks_eva, relin_keys, galois_keys, scale, ct_a, ct_c, 8,
                        16, "branch_13", &branch_traces[13]);
  });
  parallel.go([&]() {
    branches[14] =
        parallel_branch(ckks_eva, relin_keys, galois_keys, scale, ct_a, ct_d,
                        16, 32, "branch_14", &branch_traces[14]);
  });
  parallel.go([&]() {
    branches[15] =
        parallel_branch(ckks_eva, relin_keys, galois_keys, scale, ct_b, ct_c,
                        32, 1, "branch_15", &branch_traces[15]);
  });
  parallel.go([&]() {
    branches[16] =
        parallel_branch(ckks_eva, relin_keys, galois_keys, scale, ct_b, ct_d, 1,
                        2, "branch_16", &branch_traces[16]);
  });
  parallel.go([&]() {
    branches[17] =
        parallel_branch(ckks_eva, relin_keys, galois_keys, scale, ct_c, ct_d, 2,
                        4, "branch_17", &branch_traces[17]);
  });
  parallel.go([&]() {
    branches[18] =
        parallel_branch(ckks_eva, relin_keys, galois_keys, scale, ct_a, ct_b, 8,
                        16, "branch_18", &branch_traces[18]);
  });
  parallel.go([&]() {
    branches[19] =
        parallel_branch(ckks_eva, relin_keys, galois_keys, scale, ct_a, ct_c,
                        16, 32, "branch_19", &branch_traces[19]);
  });
  parallel.go([&]() {
    branches[20] =
        parallel_branch(ckks_eva, relin_keys, galois_keys, scale, ct_a, ct_d,
                        32, 1, "branch_20", &branch_traces[20]);
  });
  parallel.go([&]() {
    branches[21] =
        parallel_branch(ckks_eva, relin_keys, galois_keys, scale, ct_b, ct_c, 1,
                        2, "branch_21", &branch_traces[21]);
  });
  parallel.go([&]() {
    branches[22] =
        parallel_branch(ckks_eva, relin_keys, galois_keys, scale, ct_b, ct_d, 2,
                        4, "branch_22", &branch_traces[22]);
  });
  parallel.go([&]() {
    branches[23] =
        parallel_branch(ckks_eva, relin_keys, galois_keys, scale, ct_c, ct_d, 4,
                        8, "branch_23", &branch_traces[23]);
  });
  parallel.wait();
...
void run_example(ExecutionMode mode) {
  ...
  const std::string evaluation_label =
      std::string("CKKS DAG ") + mode_label + " 24-branch evaluation";
  ...
  const int thread_pool_workers = manual_parallel ? runtime_worker_count() : 1;
  ...
  if (manual_parallel) {
    ckks_manual_parallel_24_workload(*ckks_eva, *thread_pool, relin_keys,
                                     galois_keys, scale, ct_a, ct_b, ct_c,
