...
void record_op(std::vector<DagTraceItem> *trace, const std::string &group,
  ...
  if (trace != nullptr) {
    trace->push_back(
        {group, op, elapsed_ms(start, stop), cores, internal_omp_regions});
  }
}
...
void record_rescale_dynamic(EvaluatorCkksBase &ckks_eva, Ciphertext &cipher,
  ...
  if (trace != nullptr) {
    trace->push_back({group, "rescale_dynamic", elapsed_ms(start, stop), cores,
                      internal_omp_regions, true, level_before, cipher.level(),
...
int main() {
  ...
  const auto ckks_full_pipeline_ms = setup_ms + keygen_ms + runtime_setup_ms +
                                     message_prep_ms + encode_ms + encrypt_ms +
  ...
  std::cout << "CKKS DAG single-thread 24-branch evaluation TIME: "
            << evaluation_ms << " ms" << std::endl;
  print_stage_cores("CKKS DAG single-thread 24-branch evaluation",
                    evaluation_cores);
