...
#include "src/basics/util/globals.h"
#include "src/basics/util/mempool.h"
#include <memory>
#include <stdexcept>
...
{
/**
Manages a shared pointer to a memory pool. Poseidon uses memory pools
...
Poseidon provides overloads of the functions that take a MemoryPoolHandle
...
public:
    ...
    */
    MemoryPoolHandle(const MemoryPoolHandle &copy) noexcept { operator=(copy); }
    ...
    */
    MemoryPoolHandle(MemoryPoolHandle &&source) noexcept { operator=(std::move(source)); }
    ...
    */
    POSEIDON_NODISCARD inline static MemoryPoolHandle Global() noexcept
    {
    ...
    */
    POSEIDON_NODISCARD inline static MemoryPoolHandle ThreadLocal() noexcept
    {
    ...
    */
    POSEIDON_NODISCARD inline static MemoryPoolHandle New(bool clear_on_destruction = false)
    {
    ...
    */
    POSEIDON_NODISCARD inline operator util::MemoryPool &() const
    {
        if (!pool_)
        {
    ...
    */
    POSEIDON_NODISCARD inline std::size_t pool_count() const noexcept
    {
    ...
    */
    POSEIDON_NODISCARD inline std::size_t alloc_byte_count() const noexcept
    {
    ...
    */
    POSEIDON_NODISCARD inline long use_count() const noexcept
    {
    ...
    */
    POSEIDON_NODISCARD inline explicit operator bool() const noexcept
    {
...
the global memory pool. Poseidon uses this memory manager profile by default.
*/
...
public:
    ...
    */
    POSEIDON_NODISCARD inline virtual MemoryPoolHandle get_pool(mm_prof_opt_t) override
    {
...
public:
    ...
    */
    ...
    POSEIDON_NODISCARD static inline MemoryPoolHandle GetPool(mm_prof_opt_t prof_opt,
                                                              Args &&...args)
    {
        switch (prof_opt)
        {
    ...
    POSEIDON_NODISCARD static inline MemoryPoolHandle GetPool()
    {
        return GetPool(mm_prof_opt::mm_default);
    }

private:
    POSEIDON_NODISCARD static inline std::unique_ptr<MMProf>
    SwitchProfileThreadUnsafe(MMProf *&&mm_prof)
    {
        if (!mm_prof)
        {
    ...
    POSEIDON_NODISCARD static inline std::unique_ptr<MMProf>
    SwitchProfileThreadUnsafe(std::unique_ptr<MMProf> &&mm_prof)
    {
        if (!mm_prof)
        {
            throw std::invalid_argument("mm_prof cannot be null");
    ...
    POSEIDON_NODISCARD static inline std::unique_ptr<MMProf> &GetMMProf()
    {
...
public:
    ...
    */
    ...
    switch lock.
    */
    inline bool owns_lock() noexcept { return mm_switch_lock_.owns_lock(); }
