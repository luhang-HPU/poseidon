...
*/
void random_bytes(poseidon_byte *buf, std::size_t count);

/**
Returns a random 64-bit unsigned integer.
*/
POSEIDON_NODISCARD inline std::uint64_t random_uint64()
{
    std::uint64_t result;
...
public:
    ...
    */
    UniformRandomGeneratorInfo(prng_type type, prng_seed_type seed)
    ...
    */
    ...
    operator==(const UniformRandomGeneratorInfo &compare) const noexcept
    {
    ...
    */
    ...
    an official Poseidon PRNG type, the return value is nullptr.
    */
    ...
    */
    POSEIDON_NODISCARD inline bool has_valid_prng_type() const noexcept
    {
        switch (type_)
        {
    ...
    */
    POSEIDON_NODISCARD inline prng_type scheme() const noexcept { return type_; }

    /**
    Returns a reference to the PRNG type.
    */
    POSEIDON_NODISCARD inline prng_type &scheme() noexcept { return type_; }

    /**
    Returns a reference to the PRNG seed.
    */
    POSEIDON_NODISCARD inline const prng_seed_type &seed() const noexcept { return seed_; }

    /**
    Returns a reference to the PRNG seed.
    */
    POSEIDON_NODISCARD inline prng_seed_type &seed() noexcept { return seed_; }
    ...
    */
    POSEIDON_NODISCARD static inline std::streamoff
    SaveSize(compr_mode_type compr_mode = Serialization::compr_mode_default)
    {
        std::size_t members_size =
            Serialization::ComprSizeEstimate(sizeof(prng_type) + prng_seed_byte_count, compr_mode);
        return static_cast<std::streamoff>(sizeof(Serialization::PoseidonHeader) + members_size);
    }
    ...
    */
                               ...
                               compr_mode_type compr_mode = Serialization::compr_mode_default) const
    {
        ...
        return Serialization::Save(std::bind(&UniformRandomGeneratorInfo::save_members, this, _1),
    ...
    */
    inline std::streamoff load(const poseidon_byte *in, std::size_t size)
    {
        ...
        UniformRandomGeneratorInfo new_info;
        auto in_size = Serialization::Load(
            std::bind(&UniformRandomGeneratorInfo::load_members, &new_info, _1, _2), in, size,
            true);
        std::swap(*this, new_info);
        return in_size;
    }
...
public:
    ...
    void load_members(std::istream &stream, PoseidonVersion version);
...
public:
    ...
    */
    ...
    POSEIDON_NODISCARD inline prng_seed_type seed() const noexcept
    {
        prng_seed_type ret{};
    ...
    */
    POSEIDON_NODISCARD inline std::uint32_t generate()
    {
        std::uint32_t result;
    ...
    */
    POSEIDON_NODISCARD inline UniformRandomGeneratorInfo info() const noexcept
    {
...
protected:
    POSEIDON_NODISCARD virtual prng_type scheme() const noexcept = 0;
...
private:
    DynArray<poseidon_byte> buffer_;
...
public:
    ...
    */
    POSEIDON_NODISCARD auto create() -> std::shared_ptr<UniformRandomGenerator>
    {
        return use_random_seed_
                   ? create_impl(
                         ...
                         {
                             prng_seed_type seed;
                             random_bytes(reinterpret_cast<poseidon_byte *>(seed.data()),
    ...
    */
    POSEIDON_NODISCARD auto create(prng_seed_type seed) -> std::shared_ptr<UniformRandomGenerator>
    {
    ...
    */
    POSEIDON_NODISCARD inline bool use_random_seed() noexcept { return use_random_seed_; }
    ...
    */
    POSEIDON_NODISCARD inline prng_seed_type default_seed() noexcept { return default_seed_; }

protected:
    POSEIDON_NODISCARD virtual auto create_impl(prng_seed_type seed)
...
protected:
    POSEIDON_NODISCARD prng_type scheme() const noexcept override { return prng_type::blake2xb; }
...
protected:
    POSEIDON_NODISCARD auto create_impl(prng_seed_type seed)
...
protected:
    POSEIDON_NODISCARD prng_type scheme() const noexcept override { return prng_type::shake256; }
