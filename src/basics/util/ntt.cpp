    ...
    : public AllocatorInterface<NTT::AllocatorAdapter<seal::MemoryPoolHandle>>
{
    ...
    {
        cache_.push_back(
            static_cast<poseidon::util::MemoryPool &>(handle_).get_for_byte_count(bytes_count));
...
static intel::hexl::NTT &get_ntt(size_t N, uint64_t modulus, uint64_t root)
{
    ...
    static poseidon::util::ReaderWriterLocker ntt_cache_locker_;

    pair<uint64_t, uint64_t> key{N, modulus};

    // Enable shared access to NTT already present
    {
        poseidon::util::ReaderLock reader_lock(ntt_cache_locker_.acquire_read());
    ...
    poseidon::util::WriterLock write_lock(ntt_cache_locker_.acquire_write());
...
*/
static void compute_forward_ntt(poseidon::util::CoeffIter operand, std::size_t N,
                                std::uint64_t modulus, std::uint64_t root,
...
@param[in] output_mod_factor Bounds the output data to the range [0, output_mod_factor * modulus)
*/
static void compute_inverse_ntt(poseidon::util::CoeffIter operand, std::size_t N,
                                std::uint64_t modulus, std::uint64_t root,
                                std::uint64_t input_mod_factor, std::uint64_t output_mod_factor)
{
...
public:
    ...
    inline value_type operator*() const { return {coeff_count_power_, modulus_[index_], pool_}; }
...
void CreateNTTTables(int coeff_count_power, const vector<Modulus> &modulus,
...
void inverse_ntt_negacyclic_harvey_lazy(CoeffIter operand, const NTTTables &tables)
{
    ...
    tables.ntt_handler().transform_from_rev(operand.ptr(), tables.coeff_count_power(),
                                            tables.get_from_inv_root_powers(), &inv_degree_modulo);
#endif
}
