...
namespace util
{
template <> class Arithmetic<std::uint64_t, MultiplyUIntModOperand, MultiplyUIntModOperand>
{
public:
    Arithmetic() {}
    ...
    inline std::uint64_t add(const std::uint64_t &a, const std::uint64_t &b) const { return a + b; }
    ...
    inline MultiplyUIntModOperand mul_root_scalar(const MultiplyUIntModOperand &r,
    ...
    {
        return POSEIDON_COND_SELECT(a >= two_times_modulus_, a - two_times_modulus_, a);
    }
...
public:
    ...
    POSEIDON_NODISCARD inline std::uint64_t get_root() const { return root_; }

    POSEIDON_NODISCARD inline const MultiplyUIntModOperand *get_from_root_powers() const
    {
        return root_powers_.get();
    }

    POSEIDON_NODISCARD inline const MultiplyUIntModOperand *get_from_inv_root_powers() const
    {
        return inv_root_powers_.get();
    }

    POSEIDON_NODISCARD inline MultiplyUIntModOperand get_from_root_powers(std::size_t index) const
    {
    ...
    POSEIDON_NODISCARD inline MultiplyUIntModOperand
    ...
    POSEIDON_NODISCARD inline const MultiplyUIntModOperand &inv_degree_modulo() const
    {
        return inv_degree_modulo_;
    }

    POSEIDON_NODISCARD inline const Modulus &modulus() const { return modulus_; }

    POSEIDON_NODISCARD inline int coeff_count_power() const { return coeff_count_power_; }

    POSEIDON_NODISCARD inline std::size_t coeff_count() const { return coeff_count_; }
...
@throws std::invalid_argument if modulus is empty, modulus does not support NTT, coeff_count_power
is invalid, or pool is uninitialized.
*/
void CreateNTTTables(int coeff_count_power, const std::vector<Modulus> &modulus,
                     Pointer<NTTTables> &tables, MemoryPoolHandle pool);

void ntt_negacyclic_harvey_lazy(CoeffIter operand, const NTTTables &tables);

inline void ntt_negacyclic_harvey_lazy(RNSIter operand, std::size_t coeff_modulus_size,
                                       ConstNTTTablesIter tables)
{
#ifdef POSEIDON_DEBUG
    if (!operand)
    {
...
#endif
    POSEIDON_ITERATE(iter(operand, tables), coeff_modulus_size,
                     [&](auto I) { ntt_negacyclic_harvey_lazy(get<0>(I), get<1>(I)); });
}

inline void ntt_negacyclic_harvey_lazy(PolyIter operand, std::size_t size,
                                       ConstNTTTablesIter tables)
{
#ifdef POSEIDON_DEBUG
    if (!operand)
...
inline void ntt_negacyclic_harvey(RNSIter operand, std::size_t coeff_modulus_size,
                                  ConstNTTTablesIter tables)
{
#ifdef POSEIDON_DEBUG
    if (!operand)
...
inline void inverse_ntt_negacyclic_harvey_lazy(RNSIter operand, std::size_t coeff_modulus_size,
                                               ConstNTTTablesIter tables)
{
#ifdef POSEIDON_DEBUG
    if (!operand)
...
#endif
    POSEIDON_ITERATE(iter(operand, tables), coeff_modulus_size,
                     [&](auto I) { inverse_ntt_negacyclic_harvey_lazy(get<0>(I), get<1>(I)); });
}

inline void inverse_ntt_negacyclic_harvey_lazy(PolyIter operand, std::size_t size,
                                               ConstNTTTablesIter tables)
{
#ifdef POSEIDON_DEBUG
    if (!operand)
...
inline void inverse_ntt_negacyclic_harvey(RNSIter operand, std::size_t coeff_modulus_size,
                                          ConstNTTTablesIter tables)
{
#ifdef POSEIDON_DEBUG
    if (!operand)
...
inline void inverse_ntt_negacyclic_harvey(PolyIter operand, std::size_t size,
                                          ConstNTTTablesIter tables)
{
#ifdef POSEIDON_DEBUG
    if (!operand)
