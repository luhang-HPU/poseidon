...
#include "src/basics/util/uintarith.h"
#include <cstdint>
#include <type_traits>
...
*/
POSEIDON_NODISCARD inline std::uint64_t increment_uint_mod(std::uint64_t operand,
...
*/
POSEIDON_NODISCARD inline std::uint64_t decrement_uint_mod(std::uint64_t operand,
                                                           const Modulus &modulus)
{
#ifdef POSEIDON_DEBUG
    if (modulus.is_zero())
...
*/
POSEIDON_NODISCARD inline std::uint64_t negate_uint_mod(std::uint64_t operand,
                                                        const Modulus &modulus)
{
#ifdef POSEIDON_DEBUG
    if (modulus.is_zero())
...
*/
POSEIDON_NODISCARD inline std::uint64_t div2_uint_mod(std::uint64_t operand, const Modulus &modulus)
{
#ifdef POSEIDON_DEBUG
    if (modulus.is_zero())
    {
...
*/
POSEIDON_NODISCARD inline std::uint64_t add_uint_mod(std::uint64_t operand1, std::uint64_t operand2,
                                                     const Modulus &modulus)
{
#ifdef POSEIDON_DEBUG
    if (modulus.is_zero())
    ...
    operand1 += operand2;
    return POSEIDON_COND_SELECT(operand1 >= modulus.value(), operand1 - modulus.value(), operand1);
}
...
*/
POSEIDON_NODISCARD inline std::uint64_t sub_uint_mod(std::uint64_t operand1, std::uint64_t operand2,
                                                     const Modulus &modulus)
{
#ifdef POSEIDON_DEBUG
    if (modulus.is_zero())
    ...
    unsigned long long temp;
    std::int64_t borrow =
        static_cast<std::int64_t>(POSEIDON_SUB_BORROW_UINT64(operand1, operand2, 0, &temp));
...
POSEIDON_NODISCARD inline std::uint64_t barrett_reduce_128(const T *input, const Modulus &modulus)
{
#ifdef POSEIDON_DEBUG
    if (!input)
    {
    ...
    return POSEIDON_COND_SELECT(tmp3 >= modulus.value(), tmp3 - modulus.value(), tmp3);
}
...
*/
template <typename T, typename = std::enable_if_t<is_uint64_v<T>>>
POSEIDON_NODISCARD inline std::uint64_t barrett_reduce_64(T input, const Modulus &modulus)
{
#ifdef POSEIDON_DEBUG
    if (modulus.is_zero())
    {
    ...
    // One more subtraction is enough
    return POSEIDON_COND_SELECT(tmp[0] >= modulus.value(), tmp[0] - modulus.value(), tmp[0]);
}
...
*/
POSEIDON_NODISCARD inline std::uint64_t
multiply_uint_mod(std::uint64_t operand1, std::uint64_t operand2, const Modulus &modulus)
{
#ifdef POSEIDON_DEBUG
    if (modulus.is_zero())
    {
...
/**
This struct contains a operand and a precomputed quotient: (operand << 64) / modulus, for a specific
modulus. When passed to multiply_uint_mod, a faster variant of Barrett reduction will be performed.
...
*/
POSEIDON_NODISCARD inline std::uint64_t multiply_uint_mod(std::uint64_t x, MultiplyUIntModOperand y,
                                                          const Modulus &modulus)
{
#ifdef POSEIDON_DEBUG
    ...
    return POSEIDON_COND_SELECT(tmp2 >= p, tmp2 - p, tmp2);
}
...
*/
POSEIDON_NODISCARD inline std::uint64_t
multiply_uint_mod_lazy(std::uint64_t x, MultiplyUIntModOperand y, const Modulus &modulus)
{
#ifdef POSEIDON_DEBUG
    if (y.operand >= modulus.value())
    {
...
Correctness: Follows the condition of barrett_reduce_128.
*/
inline void modulo_uint_inplace(std::uint64_t *value, std::size_t value_uint64_count,
                                const Modulus &modulus)
{
#ifdef POSEIDON_DEBUG
    if (!value)
...
*/
POSEIDON_NODISCARD inline std::uint64_t
modulo_uint(const std::uint64_t *value, std::size_t value_uint64_count, const Modulus &modulus)
{
#ifdef POSEIDON_DEBUG
...
Correctness: Follows the condition of barrett_reduce_128.
*/
inline std::uint64_t multiply_add_uint_mod(std::uint64_t operand1, std::uint64_t operand2,
...
*/
inline std::uint64_t multiply_add_uint_mod(std::uint64_t operand1, MultiplyUIntModOperand operand2,
                                           std::uint64_t operand3, const Modulus &modulus)
{
    return add_uint_mod(multiply_uint_mod(operand1, operand2, modulus),
                        barrett_reduce_64(operand3, modulus), modulus);
}

inline bool try_invert_uint_mod(std::uint64_t operand, const Modulus &modulus,
                                std::uint64_t &result)
{
...
Correctness: Follows the condition of barrett_reduce_128.
*/
POSEIDON_NODISCARD std::uint64_t
exponentiate_uint_mod(std::uint64_t operand, std::uint64_t exponent, const Modulus &modulus);

/**
Computes numerator = numerator mod modulus, quotient = numerator / modulus.
Correctness: Follows the condition of barrett_reduce_128.
*/
void divide_uint_mod_inplace(std::uint64_t *numerator, const Modulus &modulus,
...
Correctness: Follows the condition of barrett_reduce_128.
*/
POSEIDON_NODISCARD std::uint64_t dot_product_mod(const std::uint64_t *operand1,
