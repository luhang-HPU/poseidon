...
namespace util
{
void multiply_uint(const uint64_t *operand1, size_t operand1_uint64_count, const uint64_t *operand2,
                   size_t operand2_uint64_count, size_t result_uint64_count, uint64_t *result)
{
    ...
    {
        POSEIDON_THROW(invalid_argument_error, "result_uint64_count");
    }
    if (!result)
    {
    ...
    if (result != nullptr && (operand1 == result || operand2 == result))
    {
        POSEIDON_THROW(invalid_argument_error,
                       "result cannot point to the same value as operand1 or operand2");
    }
#endif
    // Handle fast cases.
...
void multiply_uint(const uint64_t *operand1, size_t operand1_uint64_count, uint64_t operand2,
                   size_t result_uint64_count, uint64_t *result)
{
    ...
    {
        POSEIDON_THROW(invalid_argument_error, "result cannot point to the same value as operand1");
    }
#endif
    // Handle fast cases.
...
void divide_uint_inplace(uint64_t *numerator, const uint64_t *denominator, size_t uint64_count,
                         uint64_t *quotient, MemoryPool &pool)
{
#ifdef POSEIDON_DEBUG
    if (!numerator && uint64_count > 0)
    {
        POSEIDON_THROW(invalid_argument_error, "numerator");
    }
    if (!denominator && uint64_count > 0)
    {
        POSEIDON_THROW(invalid_argument_error, "denominator");
    }
    if (!quotient && uint64_count > 0)
    {
        POSEIDON_THROW(invalid_argument_error, "quotient");
    }
    ...
    {
        POSEIDON_THROW(invalid_argument_error,
                       "quotient cannot point to same value as numerator or denominator");
    }
#endif
    if (!uint64_count)
    {
...
void divide_uint128_uint64_inplace_generic(uint64_t *numerator, uint64_t denominator,
                                           uint64_t *quotient)
{
#ifdef POSEIDON_DEBUG
    if (!numerator)
    ...
    {
        POSEIDON_THROW(invalid_argument_error, "quotient cannot point to same value as numerator");
    }
#endif
