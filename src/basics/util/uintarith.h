...
POSEIDON_NODISCARD inline unsigned char
add_uint64_generic(T operand1, S operand2, unsigned char carry, unsigned long long *result)
{
...
template <typename T, typename S, typename = std::enable_if_t<is_uint64_v<T, S>>>
POSEIDON_NODISCARD inline unsigned char add_uint64(T operand1, S operand2, unsigned char carry,
                                                   unsigned long long *result)
{
    return POSEIDON_ADD_CARRY_UINT64(operand1, operand2, carry, result);
}

template <typename T, typename S, typename R, typename = std::enable_if_t<is_uint64_v<T, S, R>>>
POSEIDON_NODISCARD inline unsigned char add_uint64(T operand1, S operand2, R *result)
{
...
inline unsigned char add_uint(const std::uint64_t *operand1, std::size_t operand1_uint64_count,
                              const std::uint64_t *operand2, std::size_t operand2_uint64_count,
                              unsigned char carry, std::size_t result_uint64_count,
                              std::uint64_t *result)
{
#ifdef POSEIDON_DEBUG
    ...
    for (std::size_t i = 0; i < result_uint64_count; i++)
    {
        ...
        carry = add_uint64((i < operand1_uint64_count) ? *operand1++ : 0,
                           (i < operand2_uint64_count) ? *operand2++ : 0, carry, &temp_result);
        *result++ = temp_result;
    }
    return carry;
}

inline unsigned char add_uint(const std::uint64_t *operand1, const std::uint64_t *operand2,
...
inline unsigned char add_uint(const std::uint64_t *operand1, std::size_t uint64_count,
...
template <typename T, typename S, typename = std::enable_if_t<is_uint64_v<T, S>>>
POSEIDON_NODISCARD inline unsigned char
sub_uint64_generic(T operand1, S operand2, unsigned char borrow, unsigned long long *result)
{
#ifdef POSEIDON_DEBUG
    if (!result)
    {
...
template <typename T, typename S, typename = std::enable_if_t<is_uint64_v<T, S>>>
POSEIDON_NODISCARD inline unsigned char sub_uint64(T operand1, S operand2, unsigned char borrow,
                                                   unsigned long long *result)
{
    return POSEIDON_SUB_BORROW_UINT64(operand1, operand2, borrow, result);
}

template <typename T, typename S, typename R, typename = std::enable_if_t<is_uint64_v<T, S, R>>>
POSEIDON_NODISCARD inline unsigned char sub_uint64(T operand1, S operand2, R *result)
{
    *result = operand1 - operand2;
    return static_cast<unsigned char>(operand2 > operand1);
}

inline unsigned char sub_uint(const std::uint64_t *operand1, std::size_t operand1_uint64_count,
                              const std::uint64_t *operand2, std::size_t operand2_uint64_count,
                              unsigned char borrow, std::size_t result_uint64_count,
                              std::uint64_t *result)
{
#ifdef POSEIDON_DEBUG
    if (!result_uint64_count)
...
inline unsigned char sub_uint(const std::uint64_t *operand1, const std::uint64_t *operand2,
                              std::size_t uint64_count, std::uint64_t *result)
{
#ifdef POSEIDON_DEBUG
    if (!uint64_count)
...
inline unsigned char sub_uint(const std::uint64_t *operand1, std::size_t uint64_count,
                              std::uint64_t operand2, std::uint64_t *result)
{
#ifdef POSEIDON_DEBUG
    if (!uint64_count)
...
inline unsigned char increment_uint(const std::uint64_t *operand, std::size_t uint64_count,
                                    std::uint64_t *result)
{
#ifdef POSEIDON_DEBUG
    if (!operand)
...
inline unsigned char decrement_uint(const std::uint64_t *operand, std::size_t uint64_count,
                                    std::uint64_t *result)
{
#ifdef POSEIDON_DEBUG
...
inline void left_shift_uint128(const std::uint64_t *operand, int shift_amount,
                               std::uint64_t *result)
{
    const std::size_t bits_per_uint64_sz = static_cast<std::size_t>(bits_per_uint64);
#ifdef POSEIDON_DEBUG
...
inline void right_shift_uint128(const std::uint64_t *operand, int shift_amount,
                                std::uint64_t *result)
{
    const std::size_t bits_per_uint64_sz = static_cast<std::size_t>(bits_per_uint64);
#ifdef POSEIDON_DEBUG
...
inline void left_shift_uint192(const std::uint64_t *operand, int shift_amount,
                               std::uint64_t *result)
{
    const std::size_t bits_per_uint64_sz = static_cast<std::size_t>(bits_per_uint64);
#ifdef POSEIDON_DEBUG
...
inline void right_shift_uint192(const std::uint64_t *operand, int shift_amount,
                                std::uint64_t *result)
{
    const std::size_t bits_per_uint64_sz = static_cast<std::size_t>(bits_per_uint64);
#ifdef POSEIDON_DEBUG
...
inline void half_round_up_uint(const std::uint64_t *operand, std::size_t uint64_count,
                               std::uint64_t *result)
{
#ifdef POSEIDON_DEBUG
    if (!operand && uint64_count > 0)
...
inline void and_uint(const std::uint64_t *operand1, const std::uint64_t *operand2,
                     std::size_t uint64_count, std::uint64_t *result)
{
#ifdef POSEIDON_DEBUG
...
inline void or_uint(const std::uint64_t *operand1, const std::uint64_t *operand2,
                    std::size_t uint64_count, std::uint64_t *result)
{
#ifdef POSEIDON_DEBUG
    if (!operand1 && uint64_count > 0)
...
inline void xor_uint(const std::uint64_t *operand1, const std::uint64_t *operand2,
                     std::size_t uint64_count, std::uint64_t *result)
{
#ifdef POSEIDON_DEBUG
    if (!operand1 && uint64_count > 0)
...
{
    POSEIDON_MULTIPLY_UINT64(operand1, operand2, result128);
}
...
{
    POSEIDON_MULTIPLY_UINT64_HW64(operand1, operand2, hw64);
}

void multiply_uint(const std::uint64_t *operand1, std::size_t operand1_uint64_count,
                   const std::uint64_t *operand2, std::size_t operand2_uint64_count,
                   std::size_t result_uint64_count, std::uint64_t *result);

inline void multiply_uint(const std::uint64_t *operand1, const std::uint64_t *operand2,
                          std::size_t uint64_count, std::uint64_t *result)
{
...
void multiply_uint(const std::uint64_t *operand1, std::size_t operand1_uint64_count,
                   std::uint64_t operand2, std::size_t result_uint64_count, std::uint64_t *result);

inline void multiply_truncate_uint(const std::uint64_t *operand1, const std::uint64_t *operand2,
                                   std::size_t uint64_count, std::uint64_t *result)
{
...
template <typename T, typename = std::enable_if_t<is_uint64_v<T>>>
inline void multiply_many_uint64_except(T *operands, std::size_t count, std::size_t except,
                                        T *result, MemoryPool &pool)
{
#ifdef POSEIDON_DEBUG
    if (operands == result)
    {
    ...
    set_uint(except == 0 ? std::uint64_t(1) : static_cast<std::uint64_t>(operands[0]), count,
             result);
...
POSEIDON_FORCE_INLINE void multiply_accumulate_uint64(const std::uint64_t *operand1,
...
template <>
POSEIDON_FORCE_INLINE void
multiply_accumulate_uint64<0>(POSEIDON_MAYBE_UNUSED const std::uint64_t *operand1,
                              POSEIDON_MAYBE_UNUSED const std::uint64_t *operand2,
                              POSEIDON_MAYBE_UNUSED unsigned long long *accumulator)
{
    // Base case; nothing to do
}

void divide_uint_inplace(std::uint64_t *numerator, const std::uint64_t *denominator,
                         std::size_t uint64_count, std::uint64_t *quotient, MemoryPool &pool);

inline void divide_uint(const std::uint64_t *numerator, const std::uint64_t *denominator,
                        std::size_t uint64_count, std::uint64_t *quotient, std::uint64_t *remainder,
                        MemoryPool &pool)
{
...
void divide_uint128_uint64_inplace_generic(std::uint64_t *numerator, std::uint64_t denominator,
...
#endif
    POSEIDON_DIVIDE_UINT128_UINT64(numerator, denominator, quotient);
}

void divide_uint192_inplace(std::uint64_t *numerator, std::uint64_t denominator,
                            std::uint64_t *quotient);

POSEIDON_NODISCARD std::uint64_t exponentiate_uint_safe(std::uint64_t operand,
                                                        std::uint64_t exponent);

POSEIDON_NODISCARD std::uint64_t exponentiate_uint(std::uint64_t operand, std::uint64_t exponent);
}  // namespace util
}  // namespace poseidon
