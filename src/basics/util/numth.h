...
namespace util
{
POSEIDON_NODISCARD inline std::vector<int> naf(int value)
{
    std::vector<int> res;
...
POSEIDON_NODISCARD inline std::uint64_t gcd(std::uint64_t x, std::uint64_t y)
{
...
POSEIDON_NODISCARD inline auto xgcd(std::uint64_t x, std::uint64_t y)
...
POSEIDON_NODISCARD inline bool are_coprime(std::uint64_t x, std::uint64_t y) noexcept
{
    return !(gcd(x, y) > 1);
}

POSEIDON_NODISCARD std::vector<std::uint64_t>
multiplicative_orders(std::vector<std::uint64_t> conjugate_classes, std::uint64_t modulus);

POSEIDON_NODISCARD std::vector<std::uint64_t> conjugate_classes(std::uint64_t modulus,
...
POSEIDON_NODISCARD auto decompose_babystep_giantstep(std::uint64_t modulus, std::uint64_t input,
                                                     const std::vector<std::uint64_t> &baby_steps,
                                                     const std::vector<std::uint64_t> &giant_steps)
    -> std::pair<std::size_t, std::size_t>;

POSEIDON_NODISCARD bool is_prime(const Modulus &modulus, std::size_t num_rounds = 40);

// Generate a vector of primes with "bit_size" bits that are congruent to 1 modulo "factor"
POSEIDON_NODISCARD std::vector<Modulus> get_primes(std::uint64_t factor, int bit_size,
                                                   std::size_t count);
POSEIDON_NODISCARD std::vector<Modulus> get_primes_raise_no_check(uint64_t factor, int bit_size,
                                                                  size_t count);
// Generate one prime with "bit_size" bits that are congruent to 1 modulo "factor"
POSEIDON_NODISCARD inline Modulus get_prime(std::uint64_t factor, int bit_size)
{
