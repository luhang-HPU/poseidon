...
#include "src/basics/util/locks.h"
...
template <> class ConstPointer<poseidon_byte>;
...
{
public:
    MemoryPoolItem(poseidon_byte *data) noexcept : data_(data) {}

    POSEIDON_NODISCARD inline poseidon_byte *data() noexcept { return data_; }

    POSEIDON_NODISCARD inline const poseidon_byte *data() const noexcept { return data_; }

    POSEIDON_NODISCARD inline MemoryPoolItem *&next() noexcept { return next_; }

    POSEIDON_NODISCARD inline const MemoryPoolItem *next() const noexcept { return next_; }
...
public:
    ...
    // Byte size of the allocations (items) owned by this pool
    POSEIDON_NODISCARD inline std::size_t item_byte_count() const noexcept override
    {
        return item_byte_count_;
    }

    // Returns the total number of items allocated
    POSEIDON_NODISCARD inline std::size_t item_count() const noexcept override
    {
        return item_count_;
    }
...
public:
    ...
    POSEIDON_NODISCARD MemoryPoolItem *get() override;
...
public:
    ...
    virtual Pointer<poseidon_byte> get_for_byte_count(std::size_t byte_count) = 0;
...
public:
    ...
    POSEIDON_NODISCARD Pointer<poseidon_byte> get_for_byte_count(std::size_t byte_count) override;

    POSEIDON_NODISCARD inline std::size_t pool_count() const override
    {
        ReaderLock lock(pools_locker_.acquire_read());
        return pools_.size();
    }

    POSEIDON_NODISCARD std::size_t alloc_byte_count() const override;
...
public:
    ...
    POSEIDON_NODISCARD inline std::size_t pool_count() const override { return pools_.size(); }
