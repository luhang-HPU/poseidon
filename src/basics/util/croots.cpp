#include "src/basics/util/croots.h"
#include <complex>
    ...
    : degree_of_roots_(degree_of_roots), pool_(move(pool))
{
    ...
    if (power < 0)
    {
        POSEIDON_THROW(invalid_argument_error, "degree_of_roots must be a power of two");
    }
    else if (power < 3)
    {
        POSEIDON_THROW(invalid_argument_error, "degree_of_roots must be at least 8");
    }
#endif
...
POSEIDON_NODISCARD complex<double> ComplexRoots::get_root(size_t index) const
{
