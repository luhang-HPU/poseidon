...
namespace util
{
bool try_invert_uint_mod(const uint64_t *operand, const uint64_t *modulus, size_t uint64_count,
                         uint64_t *result, MemoryPool &pool)
{
    ...
    if (!uint64_count)
    {
        POSEIDON_THROW(invalid_argument_error, "uint64_count");
    }
    if (!result)
    {
    ...
    while (true)
    {
        ...
        size_t division_uint64_count =
            static_cast<size_t>(divide_round_up(numerator_bits, bits_per_uint64));
