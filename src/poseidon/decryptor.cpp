                            ...
                            MemoryPoolHandle pool)
{
    ...
    if (encrypted.correction_factor() != 1)
    {
        ...
        {
            POSEIDON_THROW_LOGIC_ERROR("invalid correction factor");
        }
...
void Decryptor::compute_secret_key_array(size_t max_power)
{
    ...
    {
        POSEIDON_THROW_LOGIC_ERROR("secret_key_array_ is uninitialized");
    }
#endif
...
int Decryptor::invariant_noise_budget(const Ciphertext &encrypted)
{
    ...
    {
        POSEIDON_THROW_LOGIC_ERROR("unsupported scheme");
    }
