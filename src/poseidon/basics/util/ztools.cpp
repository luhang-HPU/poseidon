...
#include "poseidon/basics/util/ztools.h"
#include <cstddef>
#include <cstring>
#include <ios>
#include <iostream>
...
public:
    ...
    {
        auto ptr = util::allocate<poseidon_byte>(size, pool_);
        void *addr = reinterpret_cast<void *>(ptr.get());
        ptr_storage_[addr] = move(ptr);
        return addr;
    }

    void free(void *addr) { ptr_storage_.erase(addr); }

private:
    MemoryPoolHandle pool_;

    unordered_map<void *, Pointer<poseidon_byte>> ptr_storage_;
};
}  // namespace
}  // namespace ztools
}  // namespace util
...
namespace
{
// The output size in a single deflate round cannot exceed 4 GB so we need to invert the
// deflateBound inequality to find an upper bound for the input size.
constexpr size_t zlib_process_bytes_out_max = static_cast<size_t>(numeric_limits<uInt>::max());

// If input size is at most zlib_process_bytes_in_max, we can complete the deflate algorithm in a
// single call to deflate (deflateBound(zlib_process_bytes_in_max) is at most 4 GB).
...
int zlib_deflate_array_inplace(DynArray<poseidon_byte> &in, MemoryPoolHandle pool)
{
    ...
    auto temp_out = DynArray<poseidon_byte>(buffer_size, pool);

    // Where we are writing output now; start writing to the temporary buffer
    poseidon_byte *out_head = temp_out.begin();
    ...
    zstream.next_in = reinterpret_cast<unsigned char *>(const_cast<poseidon_byte *>(in.cbegin()));
    ...
    do
    {
        ...
        do
        {
            ...
            while (!out_size)
            {
                ...
                if (!out_is_in)
                {
                    // If we have been writing to the temporary buffer, then see if we can copy to
                    // in
                    ...
                    {
                        // We don't have enough room to copy to in; instead, resize temp_out and
                        // continue using it, hoping that the situation will change
            ...
            process_bytes_out =
                static_cast<size_t>(reinterpret_cast<poseidon_byte *>(zstream.next_out) - out_head);
...
int zlib_inflate_stream(istream &in_stream, streamoff in_size, ostream &out_stream,
...
void zlib_write_header_deflate_buffer(DynArray<poseidon_byte> &in, void *header_ptr,
                                      ostream &out_stream, MemoryPoolHandle pool)
{
    Serialization::PoseidonHeader &header =
        *reinterpret_cast<Serialization::PoseidonHeader *>(header_ptr);
    ...
    header.size = static_cast<uint64_t>(add_safe(sizeof(Serialization::PoseidonHeader), in.size()));
    ...
    try
    {
        ...
        out_stream.write(reinterpret_cast<const char *>(&header),
                         sizeof(Serialization::PoseidonHeader));
        out_stream.write(reinterpret_cast<const char *>(in.cbegin()),
                         safe_cast<streamsize>(in.size()));
    }
    catch (...)
    {
...
namespace
{
// We cap the output size in a single compression round to 4 GB so we need to invert the
// deflateBound inequality to find an upper bound for the input size. Unlike for ZLIB, for Zstandard
// this is not a required bound. However, it can help keep the memory footprint smaller when very
// large objects are compressed.
constexpr size_t zstd_process_bytes_out_max = static_cast<size_t>(numeric_limits<uint32_t>::max());

// If input size is at most zstd_process_bytes_in_max, we can complete the deflate algorithm in a
// single call to deflate (deflateBound(zstd_process_bytes_in_max) is at most 4 GB).
...
unsigned zstd_deflate_array_inplace(DynArray<poseidon_byte> &in, MemoryPoolHandle pool)
{
    ...
    // Holds the return value of the stream compression call. This is either the amount of data that
    // remains to be flushed from internal buffers, or an error code.
    size_t pending = 0;
    ...
    do
    {
        ...
        do
        {
            ...
            // Call the stream compression; the return value indicates remaining data in internal
            // buffers, or an error code, which we need to check.
            ...
            // Continue while not all input has been read, or while there is data pending in the
            // internal buffers
...
unsigned zstd_inflate_stream(istream &in_stream, streamoff in_size, ostream &out_stream,
                             MemoryPoolHandle pool)
{
    // Clear the exception masks; this function returns an error code
    // on failure rather than throws an IO exception.
    ...
    // Holds the return value of the decompression call. This is either the amount of data that
    // remains to be flushed from internal buffers, or an error code.
    size_t pending = 0;
