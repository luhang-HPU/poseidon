...
vector<uint64_t> conjugate_classes(uint64_t modulus, uint64_t subgroup_generator)
{
    ...
    {
        POSEIDON_THROW(invalid_argument_error, "inputs too large");
    }
#endif
...
bool try_invert_uint_mod(uint64_t value, uint64_t modulus, uint64_t &result)
{
    ...
    if (modulus <= 1)
    {
        POSEIDON_THROW(invalid_argument_error, "modulus must be at least 2");
    }
#endif
    if (value == 0)
    {
...
vector<uint64_t> multiplicative_orders(vector<uint64_t> conjugate_classes, uint64_t modulus)
{
    ...
    for (uint64_t i = 2; i < modulus; i++)
    {
        ...
        {
            orders.push_back(
                orders[static_cast<size_t>(conjugate_classes[static_cast<size_t>(i)])]);
            continue;
        }
...
void babystep_giantstep(uint64_t modulus, vector<uint64_t> &baby_steps,
                        vector<uint64_t> &giant_steps)
{
    int exponent = get_power_of_two(modulus);
    if (exponent < 0)
    {
        POSEIDON_THROW(invalid_argument_error, "modulus must be a power of 2");
    }
...
pair<size_t, size_t> decompose_babystep_giantstep(uint64_t modulus, uint64_t input,
                                                  ...
                                                  const vector<uint64_t> &giant_steps)
{
    ...
    POSEIDON_THROW_LOGIC_ERROR("failed to decompose input");
}
...
vector<Modulus> get_primes(uint64_t factor, int bit_size, size_t count)
{
    ...
    if (!count)
    {
        POSEIDON_THROW(invalid_argument_error, "count must be positive");
    }
    if (bit_size > POSEIDON_MOD_BIT_COUNT_MAX || bit_size < POSEIDON_MOD_BIT_COUNT_MIN)
    {
        POSEIDON_THROW(invalid_argument_error, "bit_size is invalid");
    }
#endif
    ...
    if (count > 0)
    {
        POSEIDON_THROW_LOGIC_ERROR("failed to find enough qualifying primes");
    }
    return destination;
}

vector<Modulus> get_primes_raise_no_check(uint64_t factor, int bit_size, size_t count)
{
    auto destination = get_primes(factor, bit_size, count);
...
bool is_primitive_root(uint64_t root, uint64_t degree, const Modulus &modulus)
{
    ...
    {
        POSEIDON_THROW(invalid_argument_error, "degree must be a power of two and at least two");
    }
#endif
    if (root == 0)
    {
...
bool try_primitive_root(uint64_t degree, const Modulus &modulus, uint64_t &destination)
{
    ...
    do
    {
        ...
        destination = barrett_reduce_64(
            (static_cast<uint64_t>(rd()) << 32) | static_cast<uint64_t>(rd()), modulus);
