...
#include <stddef.h>
#include <stdint.h>
/* Poseidon edit: changed the header file path */
#include "poseidon/basics/util/fips202.h"

/* Poseidon edit: moved the rate macros here from Kyber header fips202.h */
...
/* Poseidon edit: moved the definition of keccak_state here from Kyber
 * header fips202.h */
typedef struct
{
    uint64_t s[25];
...
static void store64(uint8_t x[8], uint64_t u)
{
    ...
    for (i = 0; i < 8; i++)
        /* Poseidon edit: explicit cast to silence warnings. */
        x[i] = (uint8_t)(u >> 8 * i);
}
