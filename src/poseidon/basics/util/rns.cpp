    ...
    : pool_(move(pool)), size_(rnsbase.size())
{
    if (!size_)
    {
        POSEIDON_THROW(invalid_argument_error, "rnsbase cannot be empty");
    }
    if (!pool_)
    {
    ...
    for (size_t i = 0; i < rnsbase.size(); i++)
    {
        ...
        {
            POSEIDON_THROW(invalid_argument_error, "rnsbase is invalid");
        }
...
RNSBase::RNSBase(const RNSBase &copy, MemoryPoolHandle pool) : pool_(move(pool)), size_(copy.size_)
{
    ...
    copy_n(copy.inv_punctured_prod_mod_base_array_.get(), size_,
...
{
    bool result = false;
    POSEIDON_ITERATE(iter(base_), size_, [&](auto &I) { result = result || (I == value); });
    return result;
}
...
{
    bool result = true;
    POSEIDON_ITERATE(iter(base_), size_,
                     [&](auto &I) { result = result && superbase.contains(I); });
    return result;
}
...
RNSBase RNSBase::extend(const Modulus &value) const
{
    if (value.is_zero())
    {
        POSEIDON_THROW(invalid_argument_error, "value cannot be zero");
    }
...
bool RNSBase::initialize()
{
    ...
    if (size_ > 1)
    {
        ...
        POSEIDON_ITERATE(iter(base_, rnsbase_values), size_,
                         [&](auto I) { get<1>(I) = get<0>(I).value(); });

        // Create punctured products
        StrideIter<uint64_t *> punctured_prod(punctured_prod_array_.get(), size_);
        POSEIDON_ITERATE(iter(punctured_prod, size_t(0)), size_,
                         [&](auto I) {
                             multiply_many_uint64_except(rnsbase_values.get(), size_, get<1>(I),
                                                         get<0>(I).ptr(), pool_);
                         });
        ...
        POSEIDON_ITERATE(iter(punctured_prod, base_, inv_punctured_prod_mod_base_array_), size_,
                         [&](auto I)
                         {
...
void RNSBase::decompose(uint64_t *value, MemoryPoolHandle pool) const
{
    if (!value)
    {
        POSEIDON_THROW(invalid_argument_error, "value cannot be null");
    }
    if (!pool)
    {
        POSEIDON_THROW(invalid_argument_error, "pool is uninitialized");
    ...
    if (size_ > 1)
    {
        ...
        POSEIDON_ITERATE(iter(value, base_), size_,
                         [&](auto I)
...
void RNSBase::decompose_array(uint64_t *value, size_t count, MemoryPoolHandle pool) const
{
    ...
    if (size_ > 1)
    {
        ...
        POSEIDON_ALLOCATE_GET_STRIDE_ITER(value_copy, uint64_t, count, size_, pool);
        ...
        POSEIDON_ITERATE(iter(base_, value_out), size_,
                         [&](auto I)
                         {
                             // For each multi-precision integer in value_copy ...
                             POSEIDON_ITERATE(iter(get<1>(I), value_copy), count,
                                              [&](auto J)
                                              {
                                                  // Reduce the multi-precision integer modulo the
                                                  // base element and write to value_out
                                                  get<0>(J) =
                                                      modulo_uint(get<1>(J), size_, get<0>(I));
                                              });
                         });
    }
}
...
void RNSBase::compose(uint64_t *value, MemoryPoolHandle pool) const
{
    ...
    if (size_ > 1)
    {
        ...
        POSEIDON_ITERATE(
            iter(temp_value, inv_punctured_prod_mod_base_array_, punctured_prod, base_), size_,
            [&](auto I)
            {
...
void RNSBase::compose_array(uint64_t *value, size_t count, MemoryPoolHandle pool) const
{
    ...
    if (size_ > 1)
    {
        ...
        auto temp_mpi(allocate_uint(size_, pool));
        POSEIDON_ITERATE(
            iter(temp_array_iter, value_iter), count,
            [&](auto I)
            {
                POSEIDON_ITERATE(
                    iter(get<0>(I), inv_punctured_prod_mod_base_array_, punctured_prod, base_),
                    size_,
                    [&](auto J)
                    {
                        uint64_t temp_prod = multiply_uint_mod(get<0>(J), get<1>(J), get<3>(J));
                        multiply_uint(get<2>(J), size_, temp_prod, size_, temp_mpi.get());
                        add_uint_uint_mod(temp_mpi.get(), get<1>(I), base_prod_.get(), size_,
                                          get<1>(I));
                    });
            });
    }
...
void BaseConverter::fast_convert(ConstCoeffIter in, CoeffIter out, MemoryPoolHandle pool) const
{
    ...
    POSEIDON_ALLOCATE_GET_COEFF_ITER(temp, ibase_size, pool);
    POSEIDON_ITERATE(
    ...
    POSEIDON_ITERATE(
        iter(out, base_change_matrix_, obase_.base()), obase_size,
        [&](auto I) { get<0>(I) = dot_product_mod(temp, get<1>(I).get(), ibase_size, get<2>(I)); });
}
...
void BaseConverter::fast_convert_array(ConstRNSIter in, RNSIter out, MemoryPoolHandle pool) const
{
    ...
    {
        POSEIDON_THROW(invalid_argument_error, "in and out are incompatible");
    }
#endif
    size_t ibase_size = ibase_.size();
    size_t obase_size = obase_.size();
    ...
    POSEIDON_ALLOCATE_GET_STRIDE_ITER(temp, uint64_t, count, ibase_size, pool);
    ...
    POSEIDON_ITERATE(
        ...
        [&](auto I)
        {
            ...
            if (get<1>(I).operand == 1)
            {
                ...
                POSEIDON_ITERATE(iter(get<0>(I), temp), count,
                                 [&](auto J)
                                 {
                                     // Reduce modulo ibase element
                                     get<1>(J)[ibase_index] =
                                         barrett_reduce_64(get<0>(J), get<2>(I));
                                 });
            }
            else
            {
                ...
                POSEIDON_ITERATE(iter(get<0>(I), temp), count,
                                 [&](auto J)
                                 {
                                     ...
                                     get<1>(J)[ibase_index] =
                                         multiply_uint_mod(get<0>(J), get<1>(I), get<2>(I));
                                 });
            }
        });

    POSEIDON_ITERATE(iter(out, base_change_matrix_, obase_.base()), obase_size,
                     [&](auto I)
                     {
                         POSEIDON_ITERATE(iter(get<0>(I), temp), count,
                                          [&](auto J)
                                          {
                                              // Compute the base conversion sum modulo obase
                                              // element
                                              get<0>(J) =
                                                  dot_product_mod(get<1>(J), get<1>(I).get(),
                                                                  ibase_size, get<2>(I));
                                          });
                     });
}
...
void BaseConverter::exact_convert_array(ConstRNSIter in, CoeffIter out, MemoryPoolHandle pool) const
{
    ...
    if (obase_size != 1)
    {
        POSEIDON_THROW(invalid_argument_error, "out base in exact_convert_array must be one.");
    }
    ...
    POSEIDON_ALLOCATE_GET_STRIDE_ITER(v, double_t, count, ibase_size, pool);

    // Aggregated rounded v
    POSEIDON_ALLOCATE_GET_PTR_ITER(aggregated_rounded_v, uint64_t, count, pool);
    ...
    POSEIDON_ITERATE(
        ...
        [&](auto I)
        {
            ...
            {
                // No multiplication needed
                POSEIDON_ITERATE(iter(get<0>(I), temp, v), count,
                                 [&](auto J)
                                 {
                                     // Reduce modulo ibase element
                                     get<1>(J)[ibase_index] =
                                         barrett_reduce_64(get<0>(J), get<2>(I));
                                     double_t dividend =
                                         static_cast<double_t>(get<1>(J)[ibase_index]);
    ...
    POSEIDON_ITERATE(iter(v, aggregated_rounded_v), count,
                     [&](auto I)
                     {
    ...
    POSEIDON_ITERATE(iter(out, temp, aggregated_rounded_v), count,
                     [&](auto J)
                     {
                         // Compute the base conversion sum modulo obase element
                         auto sum_mod_obase =
                             dot_product_mod(get<1>(J), base_change_matrix_first, ibase_size, p);
                         // Minus v*[q]_{p} mod p
...
void BaseConverter::initialize()
{
    ...
    POSEIDON_ITERATE(
        iter(base_change_matrix_, obase_.base()), obase_.size(),
        [&](auto I)
        {
            // Create the base-change matrix columns
            get<0>(I) = allocate_uint(ibase_.size(), pool_);

            StrideIter<const uint64_t *> ibase_punctured_prod_array(ibase_.punctured_prod_array(),
                                                                    ibase_.size());
            POSEIDON_ITERATE(iter(get<0>(I), ibase_punctured_prod_array), ibase_.size(),
                             [&](auto J)
                             {
                                 // Base-change matrix contains the punctured products of ibase
                                 // elements modulo the obase
...
void RNSTool::initialize(size_t poly_modulus_degree, const RNSBase &q, const Modulus &t)
{
    ...
    if (q.size() < POSEIDON_COEFF_MOD_COUNT_MIN || q.size() > POSEIDON_COEFF_MOD_COUNT_MAX)
    {
        POSEIDON_THROW(invalid_argument_error, "rnsbase is invalid");
    }
    ...
    if (coeff_count_power < 0 || poly_modulus_degree > POSEIDON_POLY_MOD_DEGREE_MAX ||
        poly_modulus_degree < POSEIDON_POLY_MOD_DEGREE_MIN)
    {
        POSEIDON_THROW(invalid_argument_error, "poly_modulus_degree is invalid");
    }
    ...
    // ciphertexts are used, and n is the "delta factor" for the ring. We reserve 32 bits for K * n.
    // Here the coeff modulus primes q_i are bounded to be POSEIDON_USER_MOD_BIT_COUNT_MAX (60)
    // bits, and all primes in B and m_sk are POSEIDON_INTERNAL_MOD_BIT_COUNT (61) bits.
    ...
    auto baseconv_primes = get_primes(mul_safe(size_t(4), coeff_count_),
                                      POSEIDON_INTERNAL_MOD_BIT_COUNT, base_bsk_m_tilde_size);
    ...
    try
    {
        CreateNTTTables(coeff_count_power,
    ...
    base_q_to_m_tilde_conv_ =
        allocate<BaseConverter>(pool_, *base_q_, RNSBase({m_tilde_}, pool_), pool_);
    ...
    POSEIDON_ITERATE(iter(inv_m_tilde_mod_bsk_, base_bsk_->base()), base_bsk_size,
                     [&](auto I)
                     {
    ...
    POSEIDON_ITERATE(iter(prod_q_mod_bsk_, base_bsk_->base()), base_bsk_size,
                     [&](auto I)
    ...
    if (base_t_gamma_)
    {
        ...
        POSEIDON_ITERATE(iter(prod_t_gamma_mod_q_, base_q_->base()), base_q_size,
                         [&](auto I)
                         {
                             get<0>(I).set(multiply_uint_mod((*base_t_gamma_)[0].value(),
                                                             (*base_t_gamma_)[1].value(),
                                                             get<1>(I)),
                                           get<1>(I));
                         });
        ...
        POSEIDON_ITERATE(
            iter(neg_inv_q_mod_t_gamma_, base_t_gamma_->base()), base_t_gamma_size,
            [&](auto I)
            {
    ...
    POSEIDON_ITERATE(
        iter(inv_q_last_mod_q_, base_q_->base()), base_q_size - 1,
        [&](auto I)
        {
...
void RNSTool::divide_and_round_q_last_inplace(RNSIter input, MemoryPoolHandle pool) const
{
    ...
    {
        POSEIDON_THROW(invalid_argument_error, "input is not valid for encryption parameters");
    }
    if (!pool)
    {
        POSEIDON_THROW(invalid_argument_error, "pool is uninitialized");
    ...
    for (size_t i = 0; i < base_q_size - 1; i++)
    {
        futures.push_back(thread_pool.enqueue(
            [&, i]()
            {
                ...
                std::unique_ptr<uint64_t[]> local_temp_array(new uint64_t[coeff_count_]);
                CoeffIter local_temp(local_temp_array.get());
                ...
                sub_poly_scalar_coeffmod(local_temp, coeff_count_, half_mod, current_modulus,
                                         local_temp);

                // 3. (ct mod qi) - (ct mod qk) mod qi
                sub_poly_coeffmod(current_input, local_temp, coeff_count_, current_modulus,
                                  current_input);

                // 4. qk^(-1) * ((ct mod qi) - (ct mod qk)) mod qi
                multiply_poly_scalar_coeffmod(current_input, coeff_count_, inv_q_last,
                                              current_modulus, current_input);
            }));
    }
    for (auto &f : futures)
    {
                                                  ...
                                                  MemoryPoolHandle pool) const
{
    ...
    if (!rns_ntt_tables)
    {
        POSEIDON_THROW(invalid_argument_error, "rns_ntt_tables cannot be null");
    }
    if (!pool)
    {
        POSEIDON_THROW(invalid_argument_error, "pool is uninitialized");
...
void RNSTool::fastbconv_sk(ConstRNSIter input, RNSIter destination, MemoryPoolHandle pool) const
{
    ...
    if (!destination)
    {
        POSEIDON_THROW(invalid_argument_error, "destination cannot be null");
    }
    ...
    */
    ...
    POSEIDON_ITERATE(iter(alpha_sk, temp, input[base_b_size]), coeff_count_,
                     [&](auto I)
                     {
    ...
    POSEIDON_ITERATE(iter(prod_b_mod_q_, base_q_->base(), destination), base_q_size,
                     [&](auto I)
                     {
    ...
    */
    ...
    POSEIDON_ITERATE(
        ...
        [&](auto I)
        {
            ...
            POSEIDON_ITERATE(
                iter(get<0>(I), r_m_tilde, get<4>(I)), coeff_count_,
                [&](auto J)
                {
...
void RNSTool::fast_floor(ConstRNSIter input, RNSIter destination, MemoryPoolHandle pool) const
{
    ...
    */
    ...
    POSEIDON_ITERATE(
        ...
        [&](auto I)
        {
            POSEIDON_ITERATE(iter(get<0>(I), get<3>(I)), coeff_count_,
                             [&](auto J)
                             {
                                 // It is not necessary for the negation to be reduced modulo
                                 // base_bsk_elt
                                 get<1>(J) =
...
void RNSTool::fastbconv_m_tilde(ConstRNSIter input, RNSIter destination,
                                MemoryPoolHandle pool) const
{
#ifdef POSEIDON_DEBUG
    if (input == nullptr)
                                      ...
                                      MemoryPoolHandle pool) const
{
    ...
    POSEIDON_ALLOCATE_GET_RNS_ITER(temp, coeff_count_, base_q_size, pool);
    POSEIDON_ITERATE(iter(input, prod_t_gamma_mod_q_, base_q_->base(), temp), base_q_size,
                     [&](auto I) {
                         multiply_poly_scalar_coeffmod(get<0>(I), coeff_count_, get<1>(I),
                                                       get<2>(I), get<3>(I));
                     });
    ...
    POSEIDON_ALLOCATE_GET_RNS_ITER(temp_t_gamma, coeff_count_, base_t_gamma_size, pool);
    ...
    POSEIDON_ITERATE(
        iter(temp_t_gamma, neg_inv_q_mod_t_gamma_, base_t_gamma_->base(), temp_t_gamma),
        base_t_gamma_size,
        [&](auto I) {
    ...
    // gamma inverse mod t
    POSEIDON_ITERATE(iter(temp_t_gamma[0], temp_t_gamma[1], destination), coeff_count_,
                     [&](auto I)
                     {
                         ...
                         if (get<1>(I) > gamma_div_2)
                         {
                             ...
                             get<2>(I) = add_uint_mod(
                                 get<0>(I), barrett_reduce_64(gamma_.value() - get<1>(I), t_), t_);
                         }
                         // No correction needed
                         else
                         {
                             get<2>(I) =
                                 sub_uint_mod(get<0>(I), barrett_reduce_64(get<1>(I), t_), t_);
                         }
...
void RNSTool::mod_t_and_divide_q_last_ntt_inplace(RNSIter input, ConstNTTTablesIter rns_ntt_tables,
                                                  MemoryPoolHandle pool) const
{
    ...
    POSEIDON_ALLOCATE_ZERO_GET_COEFF_ITER(neg_c_last_mod_t, coeff_count_, pool);
    ...
    if (inv_q_last_mod_t_ != 1)
    {
        ...
        multiply_poly_scalar_coeffmod(neg_c_last_mod_t, coeff_count_, inv_q_last_mod_t_,
    ...
    auto &thread_pool = ThreadPool::get_instance();
    std::vector<std::future<void>> futures;
    futures.reserve(modulus_size - 1);
    for (size_t i = 0; i < modulus_size - 1; i++)
    {
        futures.push_back(thread_pool.enqueue(
            [&, i]()
            {
                ...
                multiply_poly_scalar_coeffmod(current_c_i, coeff_count_, inv_qk_qi, qi,
                                              current_c_i);
            }));
    }
