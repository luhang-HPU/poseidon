...
{
    constexpr std::streamsize int_max =
...
std::streamsize SafeByteBuffer::xsgetn(char_type *s, std::streamsize count)
{
    std::streamsize avail =
        std::max<>(std::streamsize(0),
                   std::min<>(count, safe_cast<std::streamsize>(std::distance(gptr(), egptr()))));
...
SafeByteBuffer::pos_type SafeByteBuffer::seekoff(off_type off, std::ios_base::seekdir dir,
                                                 std::ios_base::openmode which)
{
    // Cannot seek relative to current position for both input and
    // output heads if they are in different positions.
    if (which == (std::ios_base::in | std::ios_base::out) && dir == std::ios_base::cur &&
        gptr() != pptr())
    {
        return pos_type(off_type(-1));
    }
...
std::streamsize ArrayGetBuffer::xsgetn(char_type *s, std::streamsize count)
{
    std::streamsize avail =
        std::max<>(std::streamsize(0),
                   std::min<>(count, safe_cast<std::streamsize>(std::distance(head_, end_))));
...
ArrayGetBuffer::pos_type ArrayGetBuffer::seekoff(off_type off, std::ios_base::seekdir dir,
                                                 std::ios_base::openmode which)
{
    off_type newoff = off;
    switch (dir)
...
ArrayPutBuffer::pos_type ArrayPutBuffer::seekoff(off_type off, std::ios_base::seekdir dir,
                                                 std::ios_base::openmode which)
{
    off_type newoff = off;
    switch (dir)
