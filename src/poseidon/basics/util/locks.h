...
#ifdef POSEIDON_USE_SHARED_MUTEX
#include <mutex>
...
class POSEIDON_NODISCARD ReaderWriterLocker
{
public:
    ReaderWriterLocker() = default;

    POSEIDON_NODISCARD inline ReaderLock acquire_read() { return ReaderLock(rw_lock_mutex_); }

    POSEIDON_NODISCARD inline WriterLock acquire_write() { return WriterLock(rw_lock_mutex_); }

    POSEIDON_NODISCARD inline ReaderLock try_acquire_read() noexcept
    {
        return ReaderLock(rw_lock_mutex_, std::try_to_lock);
    }

    POSEIDON_NODISCARD inline WriterLock try_acquire_write() noexcept
    {
...
class POSEIDON_NODISCARD ReaderLock
{
public:
    ReaderLock() noexcept : locker_(nullptr) {}

    ReaderLock(ReaderLock &&move) noexcept : locker_(move.locker_) { move.locker_ = nullptr; }

    ReaderLock(ReaderWriterLocker &locker) noexcept : locker_(nullptr) { acquire(locker); }
    ...
    ~ReaderLock() noexcept { unlock(); }

    POSEIDON_NODISCARD inline bool owns_lock() const noexcept { return locker_ != nullptr; }

    void unlock() noexcept;

    inline void swap_with(ReaderLock &lock) noexcept { std::swap(locker_, lock.locker_); }
...
class POSEIDON_NODISCARD WriterLock
{
public:
    WriterLock() noexcept : locker_(nullptr) {}

    WriterLock(WriterLock &&move) noexcept : locker_(move.locker_) { move.locker_ = nullptr; }

    WriterLock(ReaderWriterLocker &locker) noexcept : locker_(nullptr) { acquire(locker); }
    ...
    ~WriterLock() noexcept { unlock(); }

    POSEIDON_NODISCARD inline bool owns_lock() const noexcept { return locker_ != nullptr; }

    void unlock() noexcept;

    inline void swap_with(WriterLock &lock) noexcept { std::swap(locker_, lock.locker_); }
...
public:
    ...
    POSEIDON_NODISCARD inline ReaderLock acquire_read() noexcept { return ReaderLock(*this); }

    POSEIDON_NODISCARD inline WriterLock acquire_write() noexcept { return WriterLock(*this); }
...
POSEIDON_NODISCARD inline bool ReaderLock::try_acquire(ReaderWriterLocker &locker) noexcept
{
    unlock();
    locker.reader_locks_.fetch_add(1, std::memory_order_acquire);
    locker_ = &locker;
...
inline void WriterLock::acquire(ReaderWriterLocker &locker) noexcept
{
    ...
    bool expected = false;
    while (
        !locker.writer_locked_.compare_exchange_strong(expected, true, std::memory_order_acquire))
    {
        expected = false;
    }
    locker_ = &locker;
...
POSEIDON_NODISCARD inline bool WriterLock::try_acquire(ReaderWriterLocker &locker) noexcept
{
    unlock();
    bool expected = false;
