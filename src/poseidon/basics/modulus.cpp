...
void Modulus::load_members(istream &stream, POSEIDON_MAYBE_UNUSED PoseidonVersion version)
{
    auto old_except_mask = stream.exceptions();
    try
    {
...
uint64_t Modulus::reduce(uint64_t value) const
{
    if (value_ == 0)
    {
        POSEIDON_THROW_LOGIC_ERROR("cannot reduce modulo a zero modulus");
    }
...
vector<Modulus> CoeffModulus::BFVDefault(size_t poly_modulus_degree, sec_level_type sec_level)
{
    ...
    {
        POSEIDON_THROW(invalid_argument_error, "non-standard poly_modulus_degree");
    }
    if (sec_level == sec_level_type::none)
    {
        POSEIDON_THROW(invalid_argument_error, "invalid security level");
    }
    ...
    default:
        POSEIDON_THROW_RUNTIME_ERROR("invalid security level");
    }
}

vector<Modulus> CoeffModulus::Create(size_t poly_modulus_degree, vector<int> bit_sizes)
{
    if (poly_modulus_degree > POSEIDON_POLY_MOD_DEGREE_MAX ||
    ...
    if (bit_sizes.size() > POSEIDON_COEFF_MOD_COUNT_MAX)
    {
        POSEIDON_THROW(invalid_argument_error, "bit_sizes is invalid");
    }
    if (accumulate(bit_sizes.cbegin(), bit_sizes.cend(), POSEIDON_USER_MOD_BIT_COUNT_MIN,
                   [](int a, int b) { return max(a, b); }) > POSEIDON_USER_MOD_BIT_COUNT_MAX ||
        accumulate(bit_sizes.cbegin(), bit_sizes.cend(), POSEIDON_USER_MOD_BIT_COUNT_MAX,
                   [](int a, int b) { return min(a, b); }) < POSEIDON_USER_MOD_BIT_COUNT_MIN)
    {
        POSEIDON_THROW(invalid_argument_error, "bit_sizes is invalid");
    }
...
vector<Modulus> CoeffModulus::Create(size_t poly_modulus_degree, const Modulus &plain_modulus,
