...
#include "poseidon/basics/util/hestdparms.h"
...
public:
    ...
    */
    Modulus(std::uint64_t value = 0) { set_value(value); }
    ...
    */
    POSEIDON_NODISCARD inline int bit_count() const noexcept { return bit_count_; }

    /**
    Returns the size (in 64-bit words) of the value of the current Modulus.
    */
    POSEIDON_NODISCARD inline std::size_t uint64_count() const noexcept { return uint64_count_; }

    /**
    Returns a const pointer to the value of the current Modulus.
    */
    POSEIDON_NODISCARD inline const uint64_t *data() const noexcept { return &value_; }

    /**
    Returns the value of the current Modulus.
    */
    POSEIDON_NODISCARD inline std::uint64_t value() const noexcept { return value_; }
    ...
    */
    POSEIDON_NODISCARD inline auto &const_ratio() const noexcept { return const_ratio_; }

    /**
    Returns whether the value of the current Modulus is zero.
    */
    POSEIDON_NODISCARD inline bool is_zero() const noexcept { return value_ == 0; }

    /**
    Returns whether the value of the current Modulus is a prime number.
    */
    POSEIDON_NODISCARD inline bool is_prime() const noexcept { return is_prime_; }
    ...
    */
    POSEIDON_NODISCARD inline bool operator==(const Modulus &compare) const noexcept
    {
    ...
    */
    POSEIDON_NODISCARD inline bool operator==(std::uint64_t compare) const noexcept
    {
    ...
    */
    ...
    save_size(compr_mode_type compr_mode = Serialization::compr_mode_default) const
    {
        std::size_t members_size =
            Serialization::ComprSizeEstimate(util::add_safe(sizeof(value_)), compr_mode);
    ...
    */
    ...
    {
        return Serialization::Save(std::bind(&Modulus::save_members, this, std::placeholders::_1),
    ...
    */
    inline std::streamoff load(std::istream &stream)
    {
        return Serialization::Load(
            std::bind(&Modulus::load_members, this, std::placeholders::_1, std::placeholders::_2),
            stream, false);
    }
    ...
    */
    POSEIDON_NODISCARD std::uint64_t reduce(std::uint64_t value) const;
...
Poseidon when constructing a POSEIDONContext object. Normal users should not
...
and is the default security level enforced by Poseidon when constructing
a POSEIDONContext object. Normal users should not have to specify the security
...
public:
    ...
    */
    POSEIDON_NODISCARD static constexpr int
    MaxBitCount(std::size_t poly_modulus_degree,
    ...
    */
    POSEIDON_NODISCARD static std::vector<Modulus>
    BFVDefault(std::size_t poly_modulus_degree, sec_level_type sec_level = sec_level_type::tc128);
    ...
    */
    POSEIDON_NODISCARD static std::vector<Modulus> Create(std::size_t poly_modulus_degree,
...
public:
    ...
    */
    POSEIDON_NODISCARD static inline Modulus Batching(std::size_t poly_modulus_degree, int bit_size)
    {
    ...
    @throws std::logic_error if not enough suitable primes could be found
    */
    POSEIDON_NODISCARD static inline std::vector<Modulus> Batching(std::size_t poly_modulus_degree,
                                                                   std::vector<int> bit_sizes)
    {
