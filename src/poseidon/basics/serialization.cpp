...
[[noreturn]] void expressive_rethrow_on_ios_base_failure(const ostream &stream)
{
    if (!stream.rdbuf())
    {
        POSEIDON_THROW_RUNTIME_ERROR("I/O error: output stream has no associated buffer");
    }
    ...
    if (typeid(rdbuf_ref).hash_code() == typeid(ArrayPutBuffer).hash_code())
    {
        ...
        if (buffer->at_end())
        {
            ...
            POSEIDON_THROW_RUNTIME_ERROR("I/O error: insufficient output buffer");
        }
    }
...
[[noreturn]] void expressive_rethrow_on_ios_base_failure(const istream &stream)
{
    if (!stream.rdbuf())
    {
        POSEIDON_THROW_RUNTIME_ERROR("I/O error: input stream has no associated buffer");
    }
    ...
    if (stream.eof())
    {
        ...
        if (typeid(rdbuf_ref).hash_code() == typeid(ArrayGetBuffer).hash_code())
        {
            ...
            POSEIDON_THROW_RUNTIME_ERROR("I/O error: input buffer ended unexpectedly");
        }
        else
        {
            // Report generic underflow
            POSEIDON_THROW_RUNTIME_ERROR("I/O error: input stream ended unexpectedly");
        }
    }
                              ...
                              istream &stream, POSEIDON_MAYBE_UNUSED bool clear_buffers)
{
    ...
    try
    {
        ...
        {
            POSEIDON_THROW_LOGIC_ERROR("incompatible version");
        }
        if (!IsValidHeader(header))
        {
            POSEIDON_THROW_LOGIC_ERROR("loaded PoseidonHeader is invalid");
        }
        ...
        case compr_mode_type::none:
            ...
            {
                POSEIDON_THROW_LOGIC_ERROR("invalid data size");
            }
            break;
#ifdef POSEIDON_USE_ZLIB
        case compr_mode_type::zlib:
        {
            ...
            if (ztools::zlib_inflate_stream(stream, safe_cast<streamoff>(compr_size), temp_stream,
                                            safe_pool))
            {
                POSEIDON_THROW_LOGIC_ERROR("stream decompression failed");
            }
