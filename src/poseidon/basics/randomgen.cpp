...
#include "poseidon/basics/util/fips202.h"
#include <algorithm>
#include <iostream>
#include <random>
...
void random_bytes(poseidon_byte *buf, size_t count)
{
    ...
    NTSTATUS status = BCryptGenRandom(NULL, reinterpret_cast<unsigned char *>(buf),
    ...
    if (!hAdvApi)
    {
        ...
        POSEIDON_THROW_RUNTIME_ERROR("Failed to load ADVAPI32.DLL");
    }
    ...
    if (!genrand_result)
    {
        ...
        POSEIDON_THROW_RUNTIME_ERROR("Failed to call RtlGenRandom");
    }
                                              ...
                                              POSEIDON_MAYBE_UNUSED PoseidonVersion version)
{
    ...
    try
    {
        ...
        {
            POSEIDON_THROW_LOGIC_ERROR("prng_type is invalid");
        }
...
void UniformRandomGenerator::generate(size_t byte_count, poseidon_byte *destination)
{
    lock_guard<mutex> lock(mutex_);
    while (byte_count)
    {
        size_t current_bytes =
            min(byte_count, static_cast<size_t>(distance(buffer_head_, buffer_end_)));
...
void Blake2xbPRNG::refill_buffer()
{
    ...
    {
        POSEIDON_THROW_RUNTIME_ERROR("blake2xb failed");
    }
    counter_++;
}
