                                   ...
                                   MemoryPoolHandle pool) const
{
    ...
    futures.reserve(rns_modulus_size);
    for (size_t I = 0; I < rns_modulus_size; ++I)
    {
        futures.push_back(thread_pool.enqueue(
            [&, I]()
            {
                ...
                // Product of two numbers is up to 60 + 60 = 120 bits, so we can sum up to 256 of
                // them without reduction.
                size_t lazy_reduction_summand_bound =
                    size_t(POSEIDON_MULTIPLY_ACCUMULATE_USER_MOD_MAX);
                    ...
                    [&](auto J)
                    {
                        ...
                        POSEIDON_ITERATE(
                            ...
                            [&](auto K)
                            {
                                ...
                                {
                                    POSEIDON_ITERATE(
                                        iter(t_operand, get<0>(K)[key_index], get<1>(K)),
                                        coeff_count,
                                        [&](auto L)
                                        {
                ...
                // PolyIter pointing to the destination t_poly_prod, shifted to the appropriate
                // modulus
