...
the size of ciphertexts back to 2. Poseidon can only relinearize size 3
...
public:
    ...
    */
    POSEIDON_NODISCARD inline static std::size_t get_index(std::size_t key_power)
    {
        if (key_power < 2)
        {
    ...
    @throws std::invalid_argument if key_power is less than 2
    */
    POSEIDON_NODISCARD inline bool has_key(std::size_t key_power) const
    {
    ...
    */
    POSEIDON_NODISCARD inline auto &key(std::size_t key_power) const
    {
