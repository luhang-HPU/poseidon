...
public:
    ...
    */
    POSEIDON_NODISCARD inline auto &data() noexcept { return sk_; }

    /**
    Returns a const reference to the underlying polynomial.
    */
    POSEIDON_NODISCARD inline auto &data() const noexcept { return sk_; }
    ...
    */
                               ...
                               compr_mode_type compr_mode = Serialization::compr_mode_default) const
    {
        ...
        return Serialization::Save(std::bind(&Plaintext::save_members, &sk_, _1),
    ...
    */
    ...
    /**
    Loads a SecretKey from an input stream overwriting the current SecretKey.
    The loaded SecretKey is verified to be valid for the given PoseidonContext.
    ...
    */
    inline std::streamoff save(poseidon_byte *out, std::size_t size,
                               compr_mode_type compr_mode = Serialization::compr_mode_default) const
    {
        return Serialization::Save(std::bind(&Plaintext::save_members, &sk_, std::placeholders::_1),
                                   sk_.save_size(compr_mode_type::none), out, size, compr_mode,
                                   true);
    }
    ...
    */
                                      ...
                                      std::size_t size)
    {
        ...
        Plaintext new_sk(MemoryManager::GetPool(mm_prof_opt::mm_force_new, true));
        auto in_size = Serialization::Load(
            std::bind(&Plaintext::load_members, &new_sk, std::move(context), _1, _2), in, size,
            true);
        std::swap(sk_, new_sk);
        return in_size;
    }
    ...
    */
    POSEIDON_NODISCARD inline auto &parms_id() noexcept { return sk_.parms_id(); }
    ...
    @see EncryptionParameters for more information about parms_id.
    */
    POSEIDON_NODISCARD inline auto &parms_id() const noexcept { return sk_.parms_id(); }

    /**
    Returns the currently used MemoryPoolHandle.
    */
    POSEIDON_NODISCARD inline MemoryPoolHandle pool() const noexcept { return sk_.pool(); }

    POSEIDON_NODISCARD inline const RNSPoly &poly() const { return sk_.polys_; }

    POSEIDON_NODISCARD inline RNSPoly &poly() { return sk_.polys_; }
